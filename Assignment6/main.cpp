...
void LoadVectorData(const char *filename) {
    ...
    for (int i = 0; i < num_scalar_fields; i++) {
        ...
        if (lower_bound < 0.0 && upper_bound > 0.0) {
            ...
            scalar_bounds[i * 2 * num_timesteps + 2 * current_timestep + 0] =
                    0.5f + scalar_bounds[i * 2 * num_timesteps + 2 * current_timestep + 0] * scale;
            scalar_bounds[i * 2 * num_timesteps + 2 * current_timestep + 1] =
                    0.5f + scalar_bounds[i * 2 * num_timesteps + 2 * current_timestep + 1] * scale;
            ...
            scalar_bounds[i * 2 * num_timesteps + 2 * current_timestep + 0] =
                    (scalar_bounds[i * 2 * num_timesteps + 2 * current_timestep + 0] + lower_bound) *
                    scale; //should be 0.0
            scalar_bounds[i * 2 * num_timesteps + 2 * current_timestep + 1] =
                    (scalar_bounds[i * 2 * num_timesteps + 2 * current_timestep + 1] + lower_bound) *
                    scale; //should be 1.0
        }
    }
...
void DownloadScalarFieldAs3DTexture(void) {
    ...
    int scalar_field_off = scalar_field_id * vol_dim[0] * vol_dim[1] *
                           num_timesteps;// + vol_dim[ 0 ] * vol_dim[ 1 ] * current_timestep;

    //download texture in correct format
    glTexImage3D(GL_TEXTURE_3D, 0, GL_INTENSITY16, vol_dim[0], vol_dim[1], num_timesteps, 0, GL_LUMINANCE, GL_FLOAT,
                 &(scalar_fields[scalar_field_off]));
}
...
void DownloadVectorFieldAs3DTexture(void) {
    ...
    //download texture in correct format
    glTexImage3D(GL_TEXTURE_3D, 0, GL_RGB16, vol_dim[0], vol_dim[1], num_timesteps, 0, GL_RGB, GL_FLOAT,
                 vector_fields_tex);
}
